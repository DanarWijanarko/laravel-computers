...
@section('container')
    <h1 class="mt-24 text-center text-3xl font-bold">{{ $header }}</h1>

    {{-- Search Start --}}
    <div class="container">
        <div class="relative mx-auto flex max-w-screen-xl justify-end">
            <form action="{{ route('articles') }}" method="get">
                <input class="header-search-input w-80 bg-transparent" type="text" placeholder="Search for Articles Name" name="search">
    ...
    @if ($articles->count())
            ...
            <div class="mx-auto max-w-screen-xl overflow-hidden rounded-lg border-2 shadow-lg">
                ...
                <div class="grid justify-items-center p-5">
                    ...
                    <p class="mb-3 text-center text-sm font-light text-slate-400">By.
                        <a href="/articles?author={{ $articles[0]->author->username }}"
                            class="font-medium text-sky-500 transition-all hover:text-sky-600">
                            {{ $articles[0]->author->name }}
                        </a>, in
                        <a href="/articles?category={{ $articles[0]->category->slug }}"
                            class="font-medium text-sky-500 transition-all hover:text-sky-600">
                    ...
                    <a href="{{ route('articleDetail', ['article' => $articles[0]->slug]) }}" class="btn-primary">
            ...
            @foreach ($articles->skip(1) as $article)
                <div class="mb-10 overflow-hidden rounded-lg border-2 shadow-lg sm:mb-0 sm:w-64 md:w-80 lg:w-72 xl:w-[26rem]">
                    <div class="flex max-h-60 items-center justify-center overflow-hidden">
                        <img class="w-full" src="http://source.unsplash.com/1000x700?{{ $article->category->name }}">
                    </div>
                    <div class="relative p-5">
                        ...
                        <a href="{{ route('articleDetail', ['article' => $article->slug]) }}"
                            class="absolute bottom-3 font-medium text-sky-500 transition-all hover:translate-x-3 hover:text-sky-300">
                            Read more<i class="fa-solid fa-arrow-right ml-1"></i>
                        </a>
                    </div>
                </div>
        ...
        {{-- Pagination End --}}
    @else
        <div class="flex h-[410px] items-center justify-center text-center text-5xl font-bold text-slate-800">
            No Articles Found
        </div>
    @endif
@endsection
